...
pub fn render_progress(s: &ProgressState, override_text: Option<&str>) -> String {
    ...
    let status = if s.cache_only {
        format!(
            "<b>Status:</b> ⚡ {} cached • 🎵 {} seeded",
            s.cached, s.ripped
        )
    } else {
        format!(
            "<b>Status:</b> ⚡ {} cached • 🎵 {} ripped",
            s.cached, s.ripped
        )
    };
    ...
    if s.failed > 0 {
        out.push_str(&format!(" • ⚠️ {} failed", s.failed));
    }
    out.push_str(&activity);
    if s.group && !s.cache_only {
        out.push_str("<br/><i>Files delivered to your private DM 📩</i>");
    }
    out
}
...
pub fn final_summary(s: &SummaryInput) -> String {
    let mut out = if s.cache_only {
        format!("✅ <b>Caching Complete!</b><br/><br/><blockquote>• <b>Target:</b> {}<br/>• <b>Total Tracks:</b> <code>{}</code><br/>• <b>Seeded to Dump:</b> 🎵 <code>{}</code> new • ⚡ <code>{}</code> already cached<br/>{}• <b>Time Elapsed:</b> <code>{}s</code><br/>• <b>Destination:</b> Dump Channel & Database</blockquote>", s.target, s.total, s.ripped, s.cached, failed_line(&s.failed), s.elapsed)
    } else if s.cached == 0 && s.ripped == 0 && s.skipped > 0 {
        format!("⚠️ <b>No Cached Tracks Available</b><br/><br/><blockquote>• <b>Target:</b> {}<br/>• <b>Total Requested:</b> <code>{}</code><br/>• <b>Skipped (Uncached):</b> 🟡 <code>{}</code><br/>• <b>Note:</b> Live ripping is currently disabled for maintenance.</blockquote>", s.target, s.total, s.skipped)
    } else {
    ...
    if s.capped > 0 {
        out.push_str(&format!(
            "<br/>ℹ️ <i>Queue was capped to {} tracks (settings limit).</i>",
            s.cap_limit
        ));
    }
    if s.group && !s.cache_only {
        out.push_str("<br/>📩 <i>All songs have been delivered to your private DM!</i>");
    }
...
fn failed_line(failed: &[(String, String)]) -> String {
    if failed.is_empty() {
        ...
        format!("• <b>Failed:</b> ⚠️ <code>{}</code><br/>", failed.len())
    }
}
...
mod tests {
    ...
    #[test]
    fn initial_and_progress_strings_match_oracle() {
        assert_eq!(
            initial_text(),
            "🔍 <b>Resolving tracks from Apple Music...</b>"
        );
        ...
        assert!(text.ends_with("<br/><i>Files delivered to your private DM 📩</i>"));
    }
    ...
    fn final_summary_has_cap_note_group_note_and_first_five_failures() {
        ...
        assert!(text.contains("Queue was capped to 5 tracks (settings limit)."));
        assert!(text.contains("All songs have been delivered to your private DM!"));
