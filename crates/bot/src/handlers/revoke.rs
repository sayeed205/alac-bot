            ...
            async move {
                ...
                match state.auth.revoke(id).await {
                    Ok(true) => {
                        ...
                        let text = format!("🗑️ <b>Revoked access for:</b> <a href=\"{link}\">{}</a> (<code>{id}</code>)", escape(&name));
                        let _ = msg.reply(InputMessage::html(parse_dynamic_html(&text))).await;
                    }
                    Ok(false) => { let _ = msg.reply(InputMessage::html(parse_dynamic_html(&format!("⚠️ <b>Not Found:</b> ID <code>{id}</code> was not in the authorized list.")))).await; }
