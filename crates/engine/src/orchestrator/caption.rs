...
pub fn format_dump_caption(meta: &DumpCaptionMetadata<'_>) -> String {
    ...
    let quote_lines = [
        ...
        format!(
            "• Apple Track ID: <b>{}</b>",
            html_escape(meta.apple_track_id)
        ),
        String::new(),
        payload_html,
    ];
...
/// parse as JSON with a string `id` (TS catch → null).
pub fn parse_dump_caption(text: Option<&str>) -> Option<ParsedDumpMetadata> {
    ...
    Some(ParsedDumpMetadata {
        apple_track_id: id.as_str().expect("checked string").to_string(),
...
fn extract_payload(text: &str) -> Option<String> {
    ...
    for (start, _) in text.match_indices('{') {
        let mut q = start + 1;
        while q <= text.len() {
            let Some(rel) = text[q..].find("\"id\"") else {
                break;
            };
            let id_at = q + rel;
            let mut i = id_at + 4; // past the literal `"id"`
...
mod tests {
    ...
    fn caption_matches_ts_shape() {
        ...
        assert!(html.contains("&quot;id&quot;: &quot;1440828878&quot;"));
    ...
    fn roundtrip_through_parse() {
        ...
        assert_eq!(parsed.apple_track_id, "1440828878");
    ...
    fn parse_rejects_missing_or_invalid() {
        ...
        // id present but not a string → null (TS checks typeof === 'string').
        assert!(parse_dump_caption(Some("{\"id\": 123}")).is_none());
    }

    #[test]
    fn parse_defaults_missing_fields() {
        let parsed = parse_dump_caption(Some("{\"id\": \"abc\"}")).expect("minimal payload parses");
        assert_eq!(parsed.apple_track_id, "abc");
