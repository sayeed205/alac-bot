...
use super::{
    ...
    playlist::{parse_master_playlist, parse_media_playlist},
};
...
impl WrapperEngine {
    ...
    pub async fn rip_track(
        ...
        // 3. Parse ALAC variant stream info
        let alac_info = parse_master_playlist(&master_text, &master_url)
            .map_err(|e| StreamError::Message(format!("Parse master playlist: {e}")))?;
